...
def get_ft_fields(simulation, ft, fields_2D=False, *slice_axis_and_which_point):
    """If boolean fields_2D is True(default is False), slice axis and point need to be specified."""
    fields_data = [simulation.get_dft_array(ft, component=field, num_freq=0) for field in
                   [mp.Ex, mp.Ey, mp.Ez, mp.Dielectric]]
    fields_data_elements = [element[1:-1, 1:-1, 1:-1] for element in fields_data]
    if fields_2D:
        slice_axis, which_point = slice_axis_and_which_point
...
def pick_max(delta, lst, lst_3D, arr_coords):
    # lst = points_to_delete
    ...
    max_x, max_y, max_z = np.unravel_index(delta.argmax(), delta.shape)

    lst.append((max_x, max_y, max_z))
    lst_3D.append((max_x, max_y, max_z))
...
# source_hg = mhg.make_hg_beam(freq, WAVELENGTH, [0, sy, sz], [SRC_POS_X, SRC_POS_Y, SRC_POS_Z], dir_prop=0, waist=WAIST,
#                              m=M, n=N)
...
def produce_simulation(src_param_arr, blk, ft_freq, ft_vol, time, obs_vol, obs_pt_arr, src_pt_arr, lsts, iter, slc_ax,
    ...
    # res = RESOLUTION
    # ft_vol = slice_volume
    # lsts = intensity_anim,intensity_at_obs,points_list
    ...
    old_field = get_ft_fields(sim, dft_obj)

    # Recording a snapshot of 2D intensity pattern for animation
    intensity_2D = get_intensity(get_ft_fields(sim, dft_obj, True, slice_axis, z_obs_index))
    ...
    delta_f = df(old_field_dft, adjoint_field_dft)
...
########################################################################################################################
    ...
    # lst = points_to_delete
    # lst_3D = points_for_3D_plot
    x_inclusion, y_inclusion, z_inclusion = pick_max(delta_f, points_to_delete, points_for_3D_plot, [x, y, z])
    ...
    for i in range(iterations):
        ...
        adjoint_field = get_ft_fields(sim_adjoint, dft_obj_adjoint)
        adjoint_field_dft = get_fields(sim_adjoint, obs_vol)
    ...
    forward_2D = get_ft_fields(sim, dft_obj, True, slice_axis, z_obs_index)
    adjoint_2D = get_ft_fields(sim_adjoint, dft_adjoint_obj, True, slice_axis, z_obs_index)

...
OBS_POSITION_X, OBS_POSITION_Y = 0, -OBS_Y_A / 2
...
ax[0, 2].plot(blocks_added, intensity_ratio)
ax[0, 2].set_title('(I at obs)/(I at source at t=0) after adding a block')
...
for ax, Si, name in zip([ax[1, 0], ax[1, 1], ax[1, 2]],
    ...
    S_ax = ax.pcolormesh(x, y, np.transpose(Si), vmax=1e-3, vmin=0, cmap='RdYlBu')

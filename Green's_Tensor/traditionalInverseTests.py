...
ax[0, 0].set_title('Ey')
ax[0, 0].pcolormesh(x, y, np.transpose(eps_data), cmap='Greys', alpha=0.9)

ax[0, 1].pcolormesh(x, y, np.transpose(ESquared))
ax[0, 1].set_title('Intensity' + str(EAtObs @ np.conjugate(EAtObs)))
ax[0, 1].pcolormesh(x, y, np.transpose(eps_data), cmap='Greys', alpha=0.9)
ax[0, 1].plot(r0[0], r0[1], 'ro')

ax[0, 2].plot(R, ESquared[r0[0], r0[1]])
ax[0, 2].set_title('Intensity at r0')

ax[1, 0].pcolormesh(x, y, np.transpose(np.real(EyA)))
ax[1, 0].set_title('EyA')
ax[1, 0].pcolormesh(x, y, np.transpose(eps_data), cmap='Greys', alpha=0.9)

ax[1, 1].pcolormesh(x, y, np.transpose(np.real(dF)))
ax[1, 1].set_title('dF')

plt.savefig("plt" + "2D plot" + ".png")
plt.close()

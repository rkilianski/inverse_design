...
def t_row_j(row_nr, src_pos_in, omega, sim_box, boundary_thickness, res, geom, pad, e_field):
...
def grid_creator(geom, simbox, boundary_thickness, pad, res):
    size, center, volume = cell_properties(simbox, boundary_thickness, pad)
    slice_vol = mp.Volume(center=center, size=volume)
    sim = mp.Simulation(cell_size=size,
                        geometry=geom,
                        geometry_center=center,
...
def t_tensor_constructor(geom, src_pos, omega, simbox, boundary_thickness, pad, res, e_field=False):
    ...
    x_grid, y_grid, z_grid = grid_creator(geom, simbox, boundary_thickness, pad, res)

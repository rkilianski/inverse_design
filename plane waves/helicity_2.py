"""Script simulating helicity lattice in vacuum with addition of macroscopic objects using 4 plane waves.
 This setup is an example of a lattice exhibiting
...
THETA = np.pi / 6
C = np.sqrt(2) / 2
K1 = C * np.array([np.sin(THETA), np.cos(THETA), 0])
K2 = C * np.array([-np.sin(THETA), np.cos(THETA), 0])
...
E2 = C * amp2 * np.array([np.cos(THETA), np.sin(THETA), 0])
...
vertices = [mp.Vector3(-COMP_X/2,COMP_X-0.2)]
GEOMETRY = [mp.Block(material=glass, size=mp.Vector3(DPML + 1, 2, 2), center=mp.Vector3(3, 3, 0)),
            mp.Block(material=glass, size=mp.Vector3(1, 1, mp.inf), center=mp.Vector3(-3, -3, 0))]
